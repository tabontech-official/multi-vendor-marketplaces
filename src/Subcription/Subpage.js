                ...
                {subscriptions.map((subscription, index) =>
                  subscription.lineItems.map((item, itemIndex) => (
                    <tr key={`${index}-${itemIndex}`} className={`border-b ${itemIndex % 2 === 0 ? 'bg-white' : 'bg-gray-100'}`}>
                      ...
                      <td className="text-sm text-gray-900 font-light px-4 py-2 whitespace-nowrap">{formatDate(subscription.createdAt)}</td>
                      <td className="text-sm text-gray-900 font-light px-4 py-2 whitespace-nowrap">{item.name}</td>
                      <td className="text-sm text-gray-900 font-light px-4 py-2 whitespace-nowrap">${item.price}</td>
                      <td className="text-sm text-gray-900 font-light px-4 py-2 whitespace-nowrap">{item.quantity}</td>
                      <td className="text-sm text-gray-900 font-light px-4 py-2 whitespace-nowrap">${(item.quantity * item.price).toFixed(2)}</td>
                      <td className="text-sm text-gray-900 font-light px-4 py-2 whitespace-nowrap">{formatDate(subscription.expiresAt)}</td>
                    </tr>
                  ))
                )}
              </tbody>
