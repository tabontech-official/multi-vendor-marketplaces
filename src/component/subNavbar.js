...
//           `http://localhost:8000/auth/getUserWithModules/${userId}`,
//         );
...
import { Link } from "react-router-dom";
import { jwtDecode } from "jwt-decode";
import { FaChevronRight, FaTags } from "react-icons/fa";
import { FiHome, FiBox, FiBarChart2, FiUsers } from "react-icons/fi";
import { FaRegClipboard } from "react-icons/fa";
        ...
        {modulesList.map((module) => {
          ...
          return (
            ...
            >
              ...
              {module.subModules && openDropdown === module.name && (
                <div className="absolute top-full left-0 bg-gray-600 text-white py-2 px-3 w-48 shadow-lg z-50">
                  {module.subModules.map((subModule) => {
                    // OTHER MENU
                    if (subModule.subModules) {
                      if (!allowedModules.includes(subModule.name)) return null;
                      return (
                        <div
                          key={subModule.name}
                          className="relative"
                          onMouseEnter={() => setOpenSubMenu(subModule.name)}
                          onMouseLeave={() => setOpenSubMenu(null)}
                        >
                          <div className="px-2 py-1 hover:bg-gray-700 cursor-pointer flex justify-between items-center">
                            <span>{subModule.name}</span>
                            <FaChevronRight className="text-xs" />
                          </div>
                          {openSubMenu === subModule.name && (
                            <div className="absolute left-full top-0 bg-gray-600 text-white py-2 px-3 w-52 shadow-lg">
