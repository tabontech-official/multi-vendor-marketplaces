    ...
    const fetchSizeCharts = async () => {
      try {
        ...
        if (role === "Dev Admin" || role === "Master Admin") {
          url = "http://localhost:8000/size-chart/all";
        } else {
          if (!userId) return;
          url = `http://localhost:8000/size-chart/all/${userId}`;
        }
  ...
  useEffect(() => {
    const fetchVendor = async () => {
      try {
        ...
        const res = await fetch(
          `http://localhost:8000/auth/getUserWithModules
/${userId}`,
          {
            method: "GET",
            headers: {
    ...
    const fetchCategories = async () => {
      ...
      try {
        const response = await fetch(
          "http://localhost:8000/category/getCategoryForProduct",
          {
            method: "GET",
            headers: {
              "x-api-key": apiKey,
  ...
  useEffect(() => {
    ...
    if ((isPopupVisible || isMediaModalVisible) && userId) {
      fetch(`http://localhost:8000/product/getImageGallery/${productId}`, {
        method: "GET",
        headers: {
          "x-api-key": apiKey,
          "x-api-secret": apiSecretKey,
  ...
  const handleImageChange = async (event) => {
    ...
    for (const [index, file] of files.entries()) {
      ...
      try {
        ...
        if (data.secure_url) {
          await fetch("http://localhost:8000/product/addImageGallery", {
            method: "POST",
            headers: {
  ...
  const uploadImagesInBackground = async ({
    ...
    try {
      ...
      const res = await fetch(
        `http://localhost:8000/product/updateImages/${productId}`,
        {
          method: "PUT",
          headers: {
            "x-api-key": apiKey,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const productRes = await fetch(
        isEditing
          ? `http://localhost:8000/product/updateProducts/${mongooseProductId}`
      ...
      //   await fetch(
      //     `http://localhost:8000/product/updateVariant/${productId}/${v.id}`,
      //     {
      //       method: "PUT",
      //       headers: {
  ...
  const handleDuplicate = async (newTitle) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/product/duplicateProduct/${product.shopifyId}`,
        {
          method: "POST",
          headers: {
            "x-api-key": apiKey,
