...
//         "http://localhost:8000/generateAcessKeys/generate-keys",
//         {
//           method: "POST",
//           headers: {
...
//       try {
//         const res = await fetch(
//           `http://localhost:8000/generateAcessKeys/getApiCredentialByUserId/${userId}`
//         );
...
const ApiCredentials = () => {
  ...
  const generateCredentials = async () => {
    try {
      const userId = localStorage.getItem("userid");
      const res = await fetch("http://localhost:8000/generateAcessKeys/generate-keys", {
        method: "POST",
  ...
  useEffect(() => {
    const fetchApiCredentials = async () => {
      ...
      if (!userId) return;
      try {
        const res = await fetch(`http://localhost:8000/generateAcessKeys/getApiCredentialByUserId/${userId}`);
        const data = await res.json();
        setApiKey(data.apiKey || "");
        setSecretKey(data.apiSecretKey || "");
