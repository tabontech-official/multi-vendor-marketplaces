  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (activeTab === "products") {
        url = isAdmin
          ? `http://localhost:8000/product/getProductWithApprovalStatus/?page=${page}&limit=${limit}`
          : `http://localhost:8000/product/getProduct/${userId}?page=${page}&limit=${limit}`;
      } else {
        url = `http://localhost:8000/users/getPendingUsers/?page=${page}&limit=${limit}`;
      }
