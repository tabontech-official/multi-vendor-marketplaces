    ...
    const fetchCategories = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // setLevel1Categories(
          //   categoryList.filter((category) => category.level === "level1"),
          // );
          setCategories(categoryList);
...
setLevel2Categories(
  categoryList.filter((category) => category.level === "level2"),
);
        } else {
...
const checkHandleExists = () => {
  ...
  return categories.some((category) => {
    const existingHandle = normalizeHandle(
      category.handle || category.title
    );

    return (
      category.level === selectedLevel &&
      (category.parentCatNo || "") === parentCatNo &&
      existingHandle === normalizedHandle
    );
  });
};
  const handleSubmit = async (e) => {
    ...
    //   return;
    // }
    if (checkHandleExists()) {
  showToast(
    "error",
    "This category already exists under the selected parent category.",
  );
  return;
}
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-5xl mx-auto p-6">
      ...
      ${
        saving
          ? "b g-gray-400 cursor-not-allowed"
