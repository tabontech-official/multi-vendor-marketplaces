    ...
    {categories
      ...
      .map((level1, index1) => {
        ...
        const level1Row = (
          <tr
            key={`l1-${index1}`}
            className={`border-b ${index1 % 2 === 0 ? "bg-white" : "bg-gray-100"}`}
          >
            <td className="p-1 text-sm">
              <input
                type="checkbox"
                ...
                onChange={(e) => {
                  if (e.target.checked) {
                    setSelectedCategoryIds((prev) => [...prev, level1._id]);
                  } else {
                    setSelectedCategoryIds((prev) =>
                      prev.filter((id) => id !== level1._id)
                    );
            ...
            {/* PRODUCT COUNT */}
            <td className="p-1 text-sm">{level1.productCount}</td>
        ...
        const level2Rows = level2s.flatMap((level2, index2) => {
          ...
          // LEVEL 2 ROW
          const level2Row = (
            <tr key={`l2-${index1}-${index2}`} className="border-b bg-gray-50">
              <td className="p-1 text-sm">
                <input
                  type="checkbox"
                  checked={selectedCategoryIds.includes(level2._id)}
                  onChange={(e) => {
                    if (e.target.checked) {
                      setSelectedCategoryIds((prev) => [...prev, level2._id]);
                    } else {
                      setSelectedCategoryIds((prev) =>
                        prev.filter((id) => id !== level2._id)
                      );
              ...
              {/* PRODUCT COUNT */}
              <td className="p-1 text-sm">{level2.productCount}</td>
            ...
            >
              <td className="p-1 text-sm">
                <input
                  ...
                  onChange={(e) => {
                    if (e.target.checked) {
                      setSelectedCategoryIds((prev) => [...prev, level3._id]);
                    } else {
                      setSelectedCategoryIds((prev) =>
                        prev.filter((id) => id !== level3._id)
                      );
              ...
              {/* PRODUCT COUNT */}
              <td className="p-1 text-sm">{level3.productCount}</td>
