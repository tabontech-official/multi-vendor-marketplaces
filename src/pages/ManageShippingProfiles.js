...
const ManageShippingProfiles = () => {
  ...
  const handleAddProfile = async () => {
    ...
    try {
      ...
      const endpoint = "http://localhost:8000/shippingProfile/add-shippings";
  ...
  const handleDeleteConfirmed = async () => {
    try {
      ...
      await axios.delete(
        `http://localhost:8000/shippingProfile/shipping-profiles/${deleteModal.id}`,
      );
  ...
  const fetchProfiles = async (userData) => {
    try {
      ...
      const endpoint = isAdmin
        ? "http://localhost:8000/shippingProfile/get/admin"
        : "http://localhost:8000/shippingProfile/getProfiles";
  ...
  const fetchUserActiveProfiles = async (userId) => {
    try {
      const { data } = await axios.get(
        `http://localhost:8000/shippingProfile/${userId}`,
      );
  ...
  const handleUserToggle = async (profile, checked) => {
    try {
      ...
      if (checked) {
        await axios.post("http://localhost:8000/shippingProfile/activate", {
        ...
        await axios.post("http://localhost:8000/shippingProfile/deactivate", {
          userId: profileUserId,
          profileId: profile.profileId,
        });
      }
  ...
  const handleAdminToggle = async (profile, checked) => {
    try {
      ...
      await axios.put(
        `http://localhost:8000/shippingProfile/updateStatus/${profile._id}`,
        {
  ...
  const handleSaveEdit = async () => {
    try {
      setLoading(true);
      await axios.put(
        `http://localhost:8000/shippingProfile/update/${editingProfile._id}`,
        editingProfile,
      );
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(
        `http://localhost:8000/shippingProfile/shipping-profiles/${id}`,
      );
      fetchProfiles();
    } catch (err) {
      console.error("Error deleting profile:", err);
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between border-b border-gray-200 pb-4 gap-4">
        <div className="flex-1">
          ...
          <p className="text-sm ml-9 text-gray-500">
            Manage your shipping profiles here.
          </p>
        </div>
      </div>
