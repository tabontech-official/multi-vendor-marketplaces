...
const MerchantPayoutDetails = () => {
  ...
  const saveBankAccount = async () => {
    ...
    try {
      const res = await axios.post(
        "http://localhost:8000/order/addPaypal",
        {
          merchantId: userId,
          payPal: account,
        }
  ...
  //         `http://localhost:8000/order/getPayoutOrders?payoutDate=${encodeURIComponent(
  //           payoutDate
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const res = await fetch(
          `http://localhost:8000/order/getPayoutByQuery?payoutDate=${encodeURIComponent(
            payoutDate
          )}&status=${status}&userId=${merchantId}`,
          {
            method: "GET",
