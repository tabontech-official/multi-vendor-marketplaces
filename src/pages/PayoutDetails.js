...
import { FaEdit, FaCheck, FaTimes, FaCog, FaDollarSign } from "react-icons/fa";
...
const PayoutDetails = () => {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      if (role === "Merchant") {
        ...
        fetch(
          `http://localhost:8000/auth/getMerchantAccountDetails/${userId}`,
        )
          .then((res) => res.json())
          .then((data) => {
            if (data?.data) {
  ...
  const fetchSubscriptions = async () => {
    ...
    try {
      const url = `http://localhost:8000/order/order/${userId}`;
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (
          ...
          url = `http://localhost:8000/order/getPayoutByQuery?payoutDate=${encodeURIComponent(
            payoutDate,
          )}&status=${status}&userId=${merchantId}`;
        } else if (userRole === "Masters") {
          url = `http://localhost:8000/order/getAllPayouts?payoutDate=${encodeURIComponent(
            payoutDate,
          )}&status=${status}`;
        } else {
  ...
  const handleSaveMerchantAccountDetails = async () => {
    try {
      ...
      const res = await fetch(
        "http://localhost:8000/auth/addMerchantAccountDetails",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleNotifyMerchant = async () => {
    try {
      ...
      const res = await fetch(
        `http://localhost:8000/auth/send-finance-reminder/${merchantId}`,
        {
          method: "POST",
  ...
  return (
    <div className="flex min-h-screen bg-[#f6f6f7] font-sans text-[#303030]">
      <SettingsSidebar />
        ...
        <div className="sticky top-0 z-20 bg-white border-b border-gray-200 px-8 py-4 flex justify-between items-center shadow-sm">
          <div className="flex items-center gap-3">
            <div className="bg-gray-100 p-2 rounded-lg text-gray-600">
              <FaDollarSign tingsOutline size={20} />
            </div>
            <div>
              ...
              <p className="text-xs text-gray-500">
                Review payout breakdown, transactions, and payment status
              </p>
            </div>
          </div>
        </div>
                  ...
                  {(userRole === "Merchant" ||
                    ...
                    <div className="text-sm text-gray-700 space-y-2">
                      {(() => {
                        ...
                        return (
                          <p className="text-sm text-gray-600 mt-2">
                            This merchant has not added payment details yet.{" "}
                            <br />
                            <span
