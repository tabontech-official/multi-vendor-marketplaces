...
const Promotion = () => {
  ...
  const fetchProductData = async () => {
    ...
    try {
      const response = await fetch(
        admin
          ? `http://localhost:8000/product/getAllDataForPromotion/?page=${page}&limit=${limit}`
          : `http://localhost:8000/product/getPromotionProduct/${id}/?page=${page}&limit=${limit}`,
        {
          method: "GET",
          headers: {
            "x-api-key": apiKey,
  ...
  //         url = "http://localhost:8000/promo/fetchAllPromotions";
  ...
  const fetchPromotions = async () => {
    try {
      ...
      if (userRole === "Merchant" || userRole === "Merchant Staff") {
        url =
          "http://localhost:8000/promo/fetchAllPromotions";
      } else if (userRole === "Dev Admin" || userRole === "Master Admin") {
        url = "http://localhost:8000/promo";
      }
  ...
  const handleSubmitPromotion = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        `http://localhost:8000/promo/${selectedVariant.id}`,
        {
          promoPrice,
  ...
  const endPromotion = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8000/promo/endPromotions/${promotionToEnd}`,
        {
          method: "DELETE",
          headers: {
            "x-api-key": apiKey,
