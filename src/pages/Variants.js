...
const Variants = () => {
  ...
  //         `http://localhost:8000/product/getSingleProductForVariants/${productId}`
  //       );
  ...
  useEffect(() => {
    ...
    const fetchVariantData = async () => {
      ...
      try {
        const response = await axios.get(
          `http://localhost:8000/product/getVariant/${productId}/variants/${variantId}`,
          {
            headers: {
              "x-api-key": apiKey,
              "x-api-secret": apiSecretKey,
  ...
  //       `http://localhost:8000/product/updateVariant/${productId}/${variantId}`,
  //       {
