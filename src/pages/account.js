...
const AccountPage = () => {
    ...
    const fetchBrandAsset = async () => {
      ...
      try {
        const res = await fetch(
          `http://localhost:8000/auth/getBrandAssets/${userId}`,
        );
  ...
  const handleSubmit2 = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:8000/auth/addBrandAsset",
        formData,
        {
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/auth/editProfile/${userId}`,
        {
          method: "PUT",
          body: form,
        },
  ...
  const updateAllProductsStatus = async (status) => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8000/product/holiday",
        {
          method: "POST",
          headers: {
            "x-api-key": apiKey,
  ...
  const handleUpdateTags = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8000/auth/createUserTagsModule",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
